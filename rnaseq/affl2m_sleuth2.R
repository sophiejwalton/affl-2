...
sample_id <- dir(file.path(base_dir, 'affl2'))
kal_dirs <- sapply(sample_id, function(id) file.path(base_dir, 'affl2', id))
s2c <- read.table(file.path(base_dir, "sleuth", "infoaffl2.txt"), header = TRUE, stringsAsFactors=FALSE)
...
so <- sleuth_fit(so, ~condition, 'full', which_var  = 'obs_counts')

so <- sleuth_fit(so, ~1, 'reduced', which_var  = 'obs_counts')
...
results_HS <- sleuth_results(so, 'reduced:full', 'lrt', show_all = FALSE)
...
write.csv(results_HS, file=file.path(base_dir, 'sleuth/results_affl2HS_goodtpm.txt'), quote=FALSE, row.names=FALSE)
